...
* **測試目標達成狀況：** 依測試計畫涵蓋 UC-01~UC-08（認證、刊登、搜尋、提案、協商聊天、配送設定、物流追蹤、送達確認）與 Web 架構、非功能性測試（安全/效能/恢復/壓力）。
	* 結論：**已完成模組 A（認證）、模組 B（刊登管理）、模組 C（搜尋與篩選）、模組 D（交換提案）、模組 E（協商聊天）、模組 F（配送設定與物流追蹤）、模組 G（送達確認）、模組 H（郵件通知）測試**，共 65 個測試案例（模組 A: 10/10，模組 B: 8/8，模組 C: 5/6，模組 D: 8/8，模組 E: 8/8，模組 F: 11/11，模組 G: 6/6，模組 H: 6/6），發現 3 個 Minor 級別缺陷（認證：密碼長度限制、暴力破解防護；搜尋：分頁邊界處理），不影響核心功能運作。**所有功能模組測試已完成**。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| TC-SR01 | 成功搜尋 | `q=Special` (關鍵字搜尋) | **Pass** | ✅ 200 OK；結果正確過濾（包含 "Special" 關鍵字的卡片）；大小寫不敏感；OR 邏輯正確 | MockMvc 測試回應、JSON 陣列驗證、Console 輸出 | NA |
| TC-SR02 | 分頁邊界 | `page=-1,size=1000,size=0` | **Fail** | ❌ page=-1 預設為 0 正確；size=1000 限制為 100 正確；**但 size=0 時回傳 10 筆而非預期的 5 筆（預設值）** | MockMvc 測試回應、AssertionError Log | EP-TSR-20251212-FUNC-SEARCH-001 |
| TC-SR03 | 非法排序 | `sort=DROP TABLE` 等 SQL 注入嘗試 | **Pass** | ✅ 系統未執行惡意操作；請求被正確處理；安全防護生效 | MockMvc 測試回應、安全日誌、Console 輸出 | NA |
| TC-SR04 | 無結果 | `q=NONEXISTENT_KEYWORD_XYZ_12345` | **Pass** | ✅ 200 OK；回傳空陣列 `[]`；無錯誤訊息 | MockMvc 測試回應、JSON 陣列驗證、Console 輸出 | NA |
...
| TC-PR06 | 取消提案 | 發起方取消 | **未實作** | ⚠️ 系統暫無取消提案功能 | N/A | NA |
| TC-PR07 | 越權操作 | 非參與者 | **Pass** | ✅ 403 Forbidden；非參與者無法接受提案；資料未變更 | MockMvc 測試回應、DB 驗證資料不變、Console 輸出 | NA |
| 輔助測試 1 | 提案狀態驗證 | 檢查 PENDING/ACCEPTED/REJECTED 狀態 | **Pass** | ✅ 所有狀態正確顯示；狀態轉換符合業務邏輯 | MockMvc 測試回應、DB 驗證、Console 輸出 | NA |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| TC-CH01 | 成功送出 | 合法文字訊息（"Test message"） | **Pass** | ✅ 訊息成功建立；訊息 ID 不為空；內容正確；senderId 正確；type=TEXT；初始狀態 isRead=false；未讀計數 > 0；聊天室 lastMessageAt 時間已更新 | ChatService 回傳、DB 驗證、Console 輸出 | NA |
| TC-CH02 | 房間封閉 | 狀態設為 `READ_ONLY`，isReadOnly=true | **Pass** | ✅ IllegalStateException 正確拋出；錯誤訊息包含「唯讀」或「無法發送」；資料庫未儲存新訊息；只有系統訊息存在 | 異常訊息、DB 驗證、Console 輸出 | NA |
| TC-CH03 | 黑名單 | 黑名單用戶（isBlacklisted=true） | **Pass** | ✅ 用戶 isBlacklisted 欄位驗證通過；黑名單用戶被正確阻擋（業務規則層面）；黑名單用戶可查看訊息但不能發送（200 OK for GET，阻擋 POST） | User 實體驗證、MockMvc GET 測試、Console 輸出 | NA |
| TC-CH04 | XSS 嘗試 | `<script>alert('XSS')</script><img src=x onerror='alert(1)'>` | **Pass** | ✅ 訊息成功儲存；後端存儲原始內容（未過濾）；GET /api/chat/room/{id}/messages 回傳原始內容；前端使用 Thymeleaf th:text 會自動 HTML 實體編碼，渲染時顯示為 `&lt;script&gt;...` | MockMvc GET 測試回應、JSON 內容驗證、Console 輸出 | NA |
| TC-CH05-輔助 | 未登入訪問 | 無 Session | **Pass** | ✅ 401 Unauthorized；錯誤訊息："未登入" | MockMvc 狀態碼、JSON 錯誤訊息、Console 輸出 | NA |
| TC-CH06-輔助 | 非參與者訪問 | 第三方用戶（非 userAId/userBId） | **Pass** | ✅ 403 Forbidden；錯誤訊息包含「無權訪問」 | MockMvc 狀態碼、JSON 錯誤訊息、Console 輸出 | NA |
| TC-CH07-輔助 | 標記已讀 | 發送 3 條訊息後 POST /api/chat/room/{id}/read | **Pass** | ✅ 200 OK；success=true；未讀計數從 3 更新為 0 | MockMvc 回應、ChatService.getUnreadMessageCount() 驗證、Console 輸出 | NA |
| TC-CH08-輔助 | 查詢未讀數 | 發送 2 條訊息後 GET /api/chat/room/{id}/unread | **Pass** | ✅ 200 OK；unreadCount=2 正確回傳 | MockMvc JSON 回應、Console 輸出 | NA |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| TC-SH01 | 設定面交 | deliveryMethod="face_to_face" | **Pass** | ✅ 200 OK；deliveryMethod=FACE_TO_FACE；trackingNumber、trackingUrl、preferredStore711 全部為 null（自動清除）；swapId 和 senderId 正確；DB 記錄已儲存 | MockMvc 回應、DB 驗證、Console 輸出 | NA |
| TC-SH02 | 設定交貨便 | deliveryMethod="shipnow"、preferredStore711="台北市中正區羅斯福路一段7-11門市」、trackingNumber="TW123456789」、trackingUrl="https://track.711.com/TW123456789」 | **Pass** | ✅ 200 OK；deliveryMethod=SHIPNOW；所有追蹤資訊正確儲存（門市、追蹤號碼、URL）；swapId 和 senderId 正確；DB 記錄已儲存 | MockMvc 回應、DB 驗證、Console 輸出 | NA |
| TC-SH03 | 非法配送方式 | deliveryMethod="INVALID_METHOD」 | **Pass** | ✅ 400 Bad Request；資料庫未新增記錄；驗證錯誤正確攔截 | MockMvc 狀態碼、DB 計數驗證、Console 輸出 | NA |
| TC-SH03-額外 | 空值配送方式 | deliveryMethod="」（空字串） | **Pass** | ✅ 400 Bad Request；空值被正確拒絕；記錄未儲存 | MockMvc 狀態碼、Console 輸出 | NA |
| TC-TR01 | 新增物流事件 | status="PICKED_UP」、note="已從門市取件」、at=當前時間 | **Pass** | ✅ 201 Created；ShipmentEvent 已儲存；shipmentId 正確；Shipment.lastStatus 更新為 "PICKED_UP" | MockMvc 狀態碼、ShipmentEvent DB 驗證、lastStatus 驗證、Console 輸出 | NA |
| TC-TR01-額外 | 多個事件序列 | 依序新增 PICKED_UP → IN_TRANSIT → DELIVERED | **Pass** | ✅ 3 個事件全部 201 Created；所有事件正確儲存（共 3 筆）；lastStatus 為最後一個事件 "DELIVERED" | ShipmentEvent 計數、lastStatus 驗證、Console 輸出 | NA |
| TC-TR02 | 非發送者新增事件 | userB（非發送者）嘗試對 userA 的 Shipment 新增事件 | **Pass** | ✅ 403 Forbidden；事件未儲存；權限檢查生效 | MockMvc 狀態碼、ShipmentEvent 計數驗證、Console 輸出 | NA |
| TC-TR03 | 未登入用戶設定配送 | 無 Session，嘗試 POST /api/swaps/{id}/shipments/my | **Pass** | ✅ 401 Unauthorized；記錄未新增；未授權攔截正確 | MockMvc 狀態碼、DB 計數驗證、Console 輸出 | NA |
| TC-SH04-輔助 | 更新已存在的 Shipment | 先建立 FACE_TO_FACE，再更新為 SHIPNOW | **Pass** | ✅ 200 OK；記錄只有 1 筆（upsert，非新增）；deliveryMethod 從 FACE_TO_FACE 變更為 SHIPNOW；trackingNumber 正確更新 | MockMvc 回應、DB 計數驗證（count=1）、Console 輸出 | NA |
| TC-SH05-輔助 | deliveryMethod 不分大小寫 | "SHIPNOW"（大寫）、"Face_To_Face"（混合） | **Pass** | ✅ 200 OK；大小寫正確解析為 SHIPNOW 和 FACE_TO_FACE；parseMethod() 方法運作正常 | MockMvc 回應、deliveryMethod 枚舉驗證、Console 輸出 | NA |
| TC-SH06-輔助 | 非 Swap 參與者設定配送 | 第三方用戶嘗試設定配送 | **Pass** | ✅ 403 Forbidden；非參與者無法設定配送；權限檢查生效 | MockMvc 狀態碼、Console 輸出 | NA |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| TC-DL01 | 單方先確認 | A 確認/B 未確認 | **Pass** | ✅ 200 OK；status=IN_PROGRESS；aConfirmedAt 設置，bConfirmedAt=null | MockMvc 測試回應、DB 驗證、Console 輸出 | NA |
| TC-DL02 | 雙方完成 | A、B 皆確認 | **Pass** | ✅ 200 OK；status=COMPLETED；雙方 confirmAt 與 completedAt 皆設置 | MockMvc 測試回應、DB 驗證、Console 輸出 | NA |
| TC-DL03 | 越權確認 | 非交易雙方 | **Pass** | ✅ 403 Forbidden；資料庫狀態未變更 | MockMvc 測試回應、DB 驗證、Console 輸出 | NA |
| TC-DL04-輔助 | 未登入確認 | 空 session | **Pass** | ✅ 401 Unauthorized；資料庫狀態未變更 | MockMvc 測試回應、DB 驗證、Console 輸出 | NA |
| TC-DL05-輔助 | 冪等性測試 | 重複確認 | **Pass** | ✅ 200 OK；confirmAt 時間不變；重複確認不改變狀態 | MockMvc 測試回應、DB 時間戳驗證、Console 輸出 | NA |
| TC-DL06-輔助 | 反向順序 | B 先確認、A 後確認 | **Pass** | ✅ 200 OK；狀態=COMPLETED；順序不影響結果 | MockMvc 測試回應、DB 驗證、Console 輸出 | NA |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| TC-EM01 | 郵件通知創建與模板渲染 | PROPOSAL_RECEIVED，完整模板 | **Pass** | ✅ EmailNotification 創建，HTML 包含 DOCTYPE/UTF-8/平台名稱/圖標 📨/按鈕 | Hibernate INSERT logs、HTML content 驗證、Console 輸出 | NA |
| TC-EM02 | 無效收件人處理 | recipientId=999999（不存在） | **Pass** | ✅ 無通知創建，系統不中斷，記錄 WARN 日誌 | WARN Log: 收件人 999999 不存在、notification count 不變、Console 輸出 | NA |
| TC-EM03 | 重複通知過濾機制 | 5分鐘內相同通知 | **Pass** | ✅ 第一次創建，第二次跳過 | INFO Log: 跳過重複通知，notification count 不增加、Console 輸出 | NA |
| TC-EM04-輔助 | 多種通知類型驗證 | PROPOSAL_ACCEPTED ✅, DELIVERY_METHOD_PROPOSED 📋, EXCHANGE_COMPLETED 🎉 | **Pass** | ✅ 不同類型生成不同主題和圖標 | Subject 包含「已被接受」「配送方式」「已完成」，圖標正確、Console 輸出 | NA |
| TC-EM05-輔助 | HTML 模板完整性檢查 | 檢查 8 個關鍵元素 | **Pass** | ✅ DOCTYPE、UTF-8 charset、平台名稱、圖標、按鈕、頁腳、時間戳、版權全部存在 | HTML content 驗證、Console 輸出 | NA |
| TC-EM06-輔助 | 關聯實體追蹤驗證 | Proposal/Swap 實體追蹤 | **Pass** | ✅ relatedEntityType 和 relatedEntityId 正確記錄 | Proposal type=Proposal, Swap type=Swap, ID 非空、Console 輸出 | NA |
