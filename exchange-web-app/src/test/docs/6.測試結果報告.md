...
| TC-PR06 | 取消提案 | 發起方取消 | **未實作** | 系統暫無取消提案功能 | N/A | NA |
...
| 輔助測試 1 | 未登入訪問 | 無 Session | **Pass** | ✅ 401 Unauthorized；錯誤訊息："未登入" | MockMvc 狀態碼、JSON 錯誤訊息、Console 輸出 | NA |
| 輔助測試 2 | 非參與者訪問 | 第三方用戶（非 userAId/userBId） | **Pass** | ✅ 403 Forbidden；錯誤訊息包含「無權訪問」 | MockMvc 狀態碼、JSON 錯誤訊息、Console 輸出 | NA |
| 輔助測試 3 | 標記已讀 | 發送 3 條訊息後 POST /api/chat/room/{id}/read | **Pass** | ✅ 200 OK；success=true；未讀計數從 3 更新為 0 | MockMvc 回應、ChatService.getUnreadMessageCount() 驗證、Console 輸出 | NA |
| 輔助測試 4 | 查詢未讀數 | 發送 2 條訊息後 GET /api/chat/room/{id}/unread | **Pass** | ✅ 200 OK；unreadCount=2 正確回傳 | MockMvc JSON 回應、Console 輸出 | NA |
...
| 輔助測試 1 | 更新已存在的 Shipment | 先建立 FACE_TO_FACE，再更新為 SHIPNOW | **Pass** | ✅ 200 OK；記錄只有 1 筆（upsert，非新增）；deliveryMethod 從 FACE_TO_FACE 變更為 SHIPNOW；trackingNumber 正確更新 | MockMvc 回應、DB 計數驗證（count=1）、Console 輸出 | NA |
| 輔助測試 2 | deliveryMethod 不分大小寫 | "SHIPNOW"（大寫）、"Face_To_Face"（混合） | **Pass** | ✅ 200 OK；大小寫正確解析為 SHIPNOW 和 FACE_TO_FACE；parseMethod() 方法運作正常 | MockMvc 回應、deliveryMethod 枚舉驗證、Console 輸出 | NA |
| 輔助測試 3 | 非 Swap 參與者設定配送 | 第三方用戶嘗試設定配送 | **Pass** | ✅ 403 Forbidden；非參與者無法設定配送；權限檢查生效 | MockMvc 狀態碼、Console 輸出 | NA |

#### 模組 G：送達確認（雙方簽收）
- **檔案/端點：** `Swap.java` (欄位：`aConfirmedAt`, `bConfirmedAt`)、`POST /api/swaps/{id}/confirm-received`
- **測試前置條件 (Pre-conditions)：**
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| TC-DL01 | 單方先確認 | A 確認/B 未確認 | Pass | 200；status=IN_PROGRESS；aConfirmedAt 設置，bConfirmedAt=null | DeliverySystemTest.testSinglePartyConfirmation() | NA |
| TC-DL02 | 雙方完成 | A、B 皆確認 | Pass | 200；status=COMPLETED；雙方 confirmAt 與 completedAt 皆設置 | DeliverySystemTest.testBothPartiesConfirmation() | NA |
| TC-DL03 | 越權確認 | 非交易雙方 | Pass | 403 Forbidden；資料庫狀態未變更 | DeliverySystemTest.testUnauthorizedConfirmation() | NA |
| 輔助測試 1 | 未登入確認 | 空 session | Pass | 401 Unauthorized；資料庫狀態未變更 | DeliverySystemTest.testUnauthenticatedConfirmation() | NA |
| 輔助測試 2 | 冪等性測試 | 重複確認 | Pass | 200 OK；confirmAt 時間不變 | DeliverySystemTest.testIdempotentConfirmation() | NA |
| 輔助測試 3 | 反向順序 | B 先確認、A 後確認 | Pass | 200 OK；狀態=COMPLETED；順序不影響結果 | DeliverySystemTest.testReversedConfirmationOrder() | NA |

#### 模組 H：郵件通知
- **檔案/端點：** EmailNotificationService、NotificationType enum、EmailNotification entity
- **測試前置條件 (Pre-conditions)：**
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| TC-EM01 | 郵件通知創建與模板渲染 | PROPOSAL_RECEIVED，完整模板 | Pass | EmailNotification 創建，HTML 包含 DOCTYPE/UTF-8/平台名稱/圖標 📨/按鈕 | Hibernate INSERT logs，HTML content 驗證 | NA |
| TC-EM02 | 無效收件人處理 | recipientId=999999（不存在） | Pass | 無通知創建，系統不中斷，記錄 WARN 日誌 | WARN: 收件人 999999 不存在，notification count 不變 | NA |
| TC-EM03 | 重複通知過濾機制 | 5分鐘內相同通知 | Pass | 第一次創建，第二次跳過 | INFO: 跳過重複通知，notification count 不增加 | NA |
| 輔助測試 1 | 多種通知類型驗證 | PROPOSAL_ACCEPTED ✅, DELIVERY_METHOD_PROPOSED 📋, EXCHANGE_COMPLETED 🎉 | Pass | 不同類型生成不同主題和圖標 | Subject 包含「已被接受」「配送方式」「已完成」，圖標正確 | NA |
| 輔助測試 2 | HTML 模板完整性檢查 | 檢查 8 個關鍵元素 | Pass | DOCTYPE, UTF-8 charset, 平台名稱, 圖標, 按鈕, 頁腳, 時間戳, 版權 | HTML content 包含所有元素 | NA |
| 輔助測試 3 | 關聯實體追蹤驗證 | Proposal/Swap 實體追蹤 | Pass | relatedEntityType 和 relatedEntityId 正確記錄 | Proposal type=Proposal, Swap type=Swap, ID 非空 | NA |
